...
resource "google_storage_bucket" "vai_managed_notebook_bucket_creation" {
  ...
  name                              = "${var.vai_managed_notebook_bucket_name}${random_string.project_random_string.id}"
...
resource "google_storage_bucket_object" "scripts_upload_to_gcs" {
  ...
  name = "${each.value}"
  bucket = "${var.vai_managed_notebook_bucket_name}${random_string.project_random_string.id}"
  depends_on = [
